...
class Request<D extends PrimaryData> {
  ...
  Request.withPayload(RequestDocument document, this.method, this.decoder,
  ...
  static Request<RelationshipObject> fetchRelationship(
          {QueryParameters parameters}) =>
      Request(HttpMethod.GET, RelationshipObject.fromJson,
          parameters: parameters);

  static Request<ResourceData> createResource(ResourceDocument document) =>
      Request.withPayload(document, HttpMethod.POST, ResourceData.fromJson);

  static Request<ResourceData> updateResource(ResourceDocument document) =>
  ...
  static Request<ToOneObject> replaceToOne(RelationshipDocument document) =>
      Request.withPayload(document, HttpMethod.PATCH, ToOneObject.fromJson);

  static Request<ToManyObject> deleteFromToMany(
          RelationshipDocument document) =>
      Request.withPayload(document, HttpMethod.DELETE, ToManyObject.fromJson);

  static Request<ToManyObject> replaceToMany(RelationshipDocument document) =>
      Request.withPayload(document, HttpMethod.PATCH, ToManyObject.fromJson);

  static Request<ToManyObject> addToMany(RelationshipDocument document) =>
